...
import { toggleWishList } from '../../../helpers/functions';
...
const Location = ({ profile, index, otherProfile }) => {

    const dispatch = useDispatch()
    const { locationResult, otherProfilesFromSeller } = useSelector(store => store.search)
    const [bookNowModel, setBookNowModel] = useState(false)

    return (
        <div className='h-100'>
            <Link to={`/search/listing/location`} state={{ profile }} className='card product-card rounded-16 p-3 h-100'>
                <div className='position-relative border rounded-10 overflow-hidden product-img mb-3 remove-position bg-secondary'>
                    ...
                    <div className={` ${profile?.wishlistProfile && 'active'} like-box d-flex justify-content-center align-items-center pointer`} onClick={(e) => {
                        e.preventDefault()
                        toggleWishList(index, profile?.profileId, profile?.profileType, otherProfile ? otherProfilesFromSeller.otherProfiles : locationResult, "LOCATION_DATA", dispatch, otherProfile)
                    }}>
