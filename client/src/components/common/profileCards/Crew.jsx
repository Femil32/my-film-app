...
const Crew = ({ profile, index, otherProfile }) => {

    const dispatch = useDispatch()
    const { crewResult, otherProfilesFromSeller } = useSelector(store => store.search)
            ...
            <Link to={`/search/listing/crew`} state={{ profile }} className='card product-card rounded-16 p-3 h-100'>
                <div className='position-relative border rounded-10 overflow-hidden product-img mb-3 remove-position bg-secondary'>
                    ...
                    <div className={` ${profile?.wishlistProfile && 'active'} like-box d-flex justify-content-center align-items-center pointer`} onClick={(e) => {
                        e.preventDefault()
                        toggleWishList(index, profile?.profileId, profile?.profileType, otherProfile ? otherProfilesFromSeller.otherProfiles : crewResult, "CREW_DATA", dispatch, otherProfile)
                    }}>
