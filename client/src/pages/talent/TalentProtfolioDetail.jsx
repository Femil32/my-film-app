...
const TalentProtfolioDetail = ({ setLoading, data }) => {
    // hooks
    ...
    useEffect(() => {
        Promise.all([
            ...
            dispatch(getTalentPortfolioPhotos(data?.profile?.profileId)),
        ]).finally(() => {
            setLoading(false)
        })
    }, [])
    ...
    useEffect(() => {
        if (talentProfile.status === 'succeed') {
            if (talentProfile.type === 'PORTFOLIO_LINK_API') {
                dispatch(setTalent())
                toast.success(commonMsg.linksUpdated)
                next()
            }
            if (talentProfile.type === 'PORTFOLIO_API') {
                dispatch(setTalent())
                setIsLoading(true)
                Promise.all([
                    setIsLoading(true),
                    isNext && dispatch(talentPortfoliolinks(portfolioLinks)),
                    dispatch(getTalentPortfolioPhotos(data?.profile?.profileId)),
                ]).finally(() => {
                    setIsLoading(false)
                })
            ...
            if (talentProfile.type === 'DEL_TALENT_PORTFOLIO_ID') {
                dispatch(setTalent())
                toast.success(commonMsg.photosDeleted)
            }
            if (talentProfile.type === 'GET_VITAL_DETAIL') {
                dispatch(setTalent())
                setAudioArray(talentProfile?.vitalDetails?.audioLinks)
                setVideoArray(talentProfile?.vitalDetails?.videoLinks)
            }
            if (talentProfile.type === 'GET_PORTFOLIO_PHOTOS') {
                dispatch(setTalent())
                let data = talentProfile?.portfolioPhotos?.portfolioImages.map(x => {
                    return {
                        ...x,
                        type: 'online',
                    }
                ...
                setTotalPages(talentProfile?.portfolioPhotos?.totalPages)
            }
        }
    }, [talentProfile])
    ...
    const handleFileChange = files => {
        ...
        Promise.all([setIsLoading(true), dispatch(postTalentPortfolio(tempdata))]).then(() => {
            setIsLoading(false)
        })
    }
    ...
    const handleImageRemove = index => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Promise.all([
                    dispatch(
                        delTalentPortfolio({ talentProfileId: data?.profile?.profileId, index })
                    ),
                ]).finally(() => {
                    dispatch(getTalentPortfolioPhotos(data?.profile?.profileId))
    ...
    const onSubmitForm = () => {
        ...
        dispatch(talentPortfoliolinks(payloadLinks))
    }
...
export default IsLoadingHOC(TalentProtfolioDetail)
