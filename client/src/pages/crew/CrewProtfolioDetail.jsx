...
// import { string } from 'yup';  For url validation

const CrewProtfolioDetail = ({ setLoading, data }) => {
    // hooks
    const { next } = useContext(CrewContext)
    ...
    useEffect(() => {
        Promise.all([
            setLoading(true),
            dispatch(getCrewPortfolioPhotos(data?.profile?.profileId)),
            dispatch(getcrewPortfolioLinks(data?.profile?.profileId)),
        ]).finally(() => {
            setLoading(false)
        })
    }, [])
    ...
    useEffect(() => {
        if (crewProfile.status === 'succeed') {
            if (crewProfile.type === 'PORTFOLIO_LINK_API') {
                dispatch(setCrew())
                toast.success(commonMsg.linksUpdated)
                next()
            }
            if (crewProfile.type === 'PORTFOLIO_API') {
                dispatch(setCrew())
                setIsLoading(true)
                Promise.all([
                    setIsLoading(true),
                    isNext && dispatch(crewPortfolioLinks(portfolioLinks)),
                    dispatch(getCrewPortfolioPhotos(data?.profile?.profileId)),
                ]).finally(() => {
                    setIsLoading(false)
                })
            ...
            if (crewProfile.type === 'DEL_CREW_PORTFOLIO_ID') {
                dispatch(setCrew())
                toast.success(commonMsg.photosDeleted)
            }
            if (crewProfile.type === 'GET_LINKS') {
                dispatch(setCrew())
                setAudioArray(crewProfile?.crew_portfolio_links?.audioLinks)
                setVideoArray(crewProfile?.crew_portfolio_links?.videoLinks)
            }
            if (crewProfile.type === 'GET_PORTFOLIO_PHOTOS') {
                dispatch(setCrew())
                let data = crewProfile?.portfolioPhotos?.portfolioImages.map(x => {
                    return {
                        ...x,
                        type: 'online',
                    }
                ...
                setTotalPages(crewProfile?.portfolioPhotos?.totalPages)
            }
        }
    }, [crewProfile])
    ...
    const handleFileChange = files => {
        ...
        Promise.all([setIsLoading(true), dispatch(postCrewPortfolio(tempdata))]).finally(() => {
            setIsLoading(false)
        })
    }
    ...
    const handleImageRemove = index => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // setIsLoading(true)
                Promise.all([
                    dispatch(delCrewPortfolio({ crewProfileId: data?.profile?.profileId, index })),
                ]).finally(() => {
                    dispatch(getCrewPortfolioPhotos(data?.profile?.profileId))
    ...
    const onSubmitForm = () => {
        ...
        dispatch(crewPortfolioLinks(payloadLinks))
    }
                                    ...
                                    allProfilesPhotos.map((element, index) => (
                                        ...
                                        >
                                            {isLoading ? (
                                                ...
                                                <a
                                                    href={element.imageUrl || element.url}
...
export default IsLoadingHOC(CrewProtfolioDetail)
