...
const TalentProfile = ({ setLoading }) => {
    const { otherTalentListing, dispatchSearch } = useSearchContext()
    // hooks
    ...
    const toggleWishTalentList = (index, profileId, profileType) => {
        if (otherTalentListing[index]?.wishlistProfile) {
            dispatch(deleteWishlistItem(otherTalentListing[index]?.wishlistItemId))
            let temp = [...otherTalentListing]
            ...
            dispatch(addWishlist({ profileId, profileType })).then(res => {
                let temp = [...otherTalentListing]
                temp[index] = { ...temp[index], wishlistProfile: true, wishlistItemId: res?.payload?.data?.wishlistItemId }
                dispatchSearch({
                    type: "OTHER_TALENT_DATA",
                    payload: temp
                })
            ...
            <Container>
                ...
                <CurrentDirectorylink className={'my-3'} routeData={[
                    ...
                    },
                    {
                        name: 'listing',
                        link: '/search/listing',
                        state: { keyword: "talent", keywordId: 1 }
                    },
                    {
                        name: 'talent-profile',
                        link: '/search/listing/talent'
                    },
                ]} />
                <div className='row'>
                    ...
                    <div id='overview'>
                        ...
                        <div className='py-3 border-bottom'>
                            ...
                            <PTag
                                ...
                                texts={profileDetails?.profileDescription}
                            />
                        </div>
                        <div className='py-3 border-bottom'>
                            <PTag
                                ...
                                texts={'Vitals'}
                            />
                            <div className='d-flex gap-5 text-capitalize'>
                                {Object.keys(profileDetails?.credentials).map((key, i) => {
                                    return (
                                        (profileDetails?.credentials[key]?.length > 0) ? <div>
                                            <p key={i} className='mb-2 opacity-75'>{key}</p>
                                            {Array.isArray(profileDetails?.credentials[key]) ? profileDetails?.credentials[key].map((data, ii) => <p key={ii} className='text-capitalize fw-bold fn-16'>{data}</p>) : <p key={i}>{profileDetails?.credentials[key]}</p>}
                                        </div> : i === 0 && "NA"
                                    )
                                })}
                            </div>
                        ...
                        <div className='py-3 border-bottom'>
                            ...
                            {profileDetails?.rates?.map((rate, i) => {
                                return (
                                    <div className='d-flex fn-3 justify-content-between align-items-center mb-2' key={i} >
                                ...
                                >
                                    {otherProfilesFromSeller?.otherProfiles?.map((data, i) => (
                                        <SwiperSlide key={i} className="h-unset">
                                            <Talent profile={data} key={i} index={i} otherProfile={true} />
                                        </SwiperSlide>
                                    ))}
                                </Swiper>
                            </div>
...
export default IsLoadingHOC(TalentProfile)
