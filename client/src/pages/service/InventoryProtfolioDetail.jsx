...
const InventoryProtfolioDetail = ({ setLoading, data }) => {
    // hooks
    const { next, inventory } = useContext(InventoryContext)
    const dispatch = useDispatch()
    const serviceProfile = useSelector(state => state.service)
    ...
    useEffect(() => {
        Promise.all([
            setLoading(true),
            data.isEdit &&
            (dispatch(getServicePortfolioLinks(inventory?.serviceInventoryId)),
                dispatch(getServicePortfolioPhotos(inventory?.serviceInventoryId))),
        ]).finally(() => {
            setLoading(false)
        })
    }, [])
    ...
    useEffect(() => {
        if (serviceProfile.status === 'succeed') {
            if (serviceProfile.type === 'PORTFOLIO_LINK_API') {
                dispatch(setService())
                toast.success(commonMsg.linksUpdated)
                next()
            }
            if (serviceProfile.type === 'PORTFOLIO_API') {
                dispatch(setService())
                setIsLoading(true)
                Promise.all([
                    setIsLoading(true),
                    isNext && dispatch(servicePortfolioLinks(portfolioLinks)),
                    data.isEdit &&
                    dispatch(getServicePortfolioPhotos(inventory?.serviceInventoryId)),
                ]).finally(() => {
                    setIsLoading(false)
            ...
            if (serviceProfile.type === 'DEL_SERVICE_PORTFOLIO_ID') {
                dispatch(setService())
                toast.success(commonMsg.photosDeleted)
            }
            if (serviceProfile.type === 'GET_LINKS') {
                dispatch(setService())
                setAudioArray(serviceProfile?.service_portfolio_links?.audioLinks)
                setVideoArray(serviceProfile?.service_portfolio_links?.videoLinks)
            }
            if (serviceProfile.type === 'GET_PORTFOLIO_PHOTOS') {
                let data = serviceProfile?.portfolioPhotos?.portfolioImages.map(x => {
                    return {
                        ...x,
                        type: 'online',
                    }
                ...
                setTotalPages(serviceProfile?.portfolioPhotos?.totalPages)
            }
        }
    }, [serviceProfile])
    ...
    useEffect(() => {
        const lightbox = new PhotoSwipeLightbox({
            gallery: '#service-profile-photos',
            children: 'a',
    ...
    const handleFileChange = files => {
        ...
        Promise.all([setIsLoading(true), dispatch(postServicePortfolio(tempdata))]).finally(() => {
            setIsLoading(false)
        })
    }
    ...
    const handleImageRemove = index => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Promise.all([
                    dispatch(
                        delServicePortfolio({ inventoryId: inventory?.serviceInventoryId, index })
                    ),
                ]).finally(() => {
                    dispatch(getServicePortfolioPhotos(inventory?.serviceInventoryId))
    ...
    const onSubmitForm = () => {
        ...
        dispatch(servicePortfolioLinks(payloadLinks))
    }
...
export default IsLoadingHOC(InventoryProtfolioDetail)
